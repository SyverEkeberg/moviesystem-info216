...
g.parse('env/static/rdf/moviedata_turtle4',format='turtle')
...
def home():
    ...
    if form.validate_on_submit():
        test = form.directorName.data
        session["TEST"] = test
        
       
        return render_template('index.html',movie=alltitles_query(), specific=specific_query("5",session["TEST"]), form=form)
    
    return render_template('index.html', movie=alltitles_query(), form=form, specific=specific_query(" "," "),directors = alldirectors_query())
